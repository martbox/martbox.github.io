var Xonomy={
	lang: "", //"en"|"de"|fr"| ...
	mode: "nerd", //"nerd"|"laic"
	slideSpeed: 100  //added to tailor the speed of jquery slideDown
};
...
/**build js object model of the given xml string/document object
e.g.  Xonomy.xml2js(xml, null, {});
 gives you object like this..
 {
 internalParent : don't use
 ...
 htmlID : ""   //populated only when harvested from xonomy editor UI I think
 ...
 //util methods that elements have..
 hasAttribute()
 getAttribute()
 getAttributeValue()
 hasChildElement()
...
*/
Xonomy.xml2js=function(xml, _jsParent, _namespaces) {
	if(typeof(xml)=="string") xml=$.parseXML(xml);
	if(xml.documentElement) xml=xml.documentElement;
	var namespaces = _namespaces || Xonomy.namespaces;
  var jsParent = _jsParent || null;
	...
	for(var i=0; i<xml.attributes.length; i++) {
		...
		if(!Xonomy.isNamespaceDeclaration(attr.nodeName)) {
			...
			namespaces[attr.nodeName]=attr.value;
		}
	}
	js.children=[];
	for(var i=0; i<xml.childNodes.length; i++) {
		...
		if(child.nodeType==1) { //element node
			js["children"].push(Xonomy.xml2js(child, js, namespaces));
		}
...
Xonomy.enrichElement=function(jsElement) {
	...
	//returns all element type children matching given name, or all if no name given
	jsElement.getChildElements=function(name) {
		var ret=[];
		for(var i=0; i<this.children.length; i++) {
			if(this.children[i].type=="element" && (typeof name=="undefined" || this.children[i].name==name)) ret.push(this.children[i]);
		}
		return ret;
	};
...
Xonomy.verifyDocSpecElement=function(name) { //make sure the DocSpec object has such an element, that the element has everything it needs
	if(!Xonomy.docSpec.elements[name] || typeof(Xonomy.docSpec.elements[name])!="object") {
		 Xonomy.docSpec.elements[name]=Xonomy.docSpec.defaultElementSpec || {}; //use default element spec if available
	}
...
Xonomy.keyHandler=function(evt){
 var KEYDOWN = {ENTER: 13, TAB: 9, SPACE: 32, ESC:27, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40, C:67, V:86};
 var key = evt.which;
 //console.log("key: "+key);
 var $element = $(evt.target);
 if($('#xonomyBubbleContent').length==0) { //popup closed
	var tabindex = Number($element.attr("tabindex") || 0);
	if(evt.ctrlKey) { //control key pressed
		if([KEYDOWN.SPACE].indexOf(key)>-1) {
			//click closest +/- element up the tree to expand/collapse
			$element.closest("div.element").find("span.plusminus").click();
		}
		else if(key==KEYDOWN.C) {
			Xonomy.copyElement($element.closest("div.element").get(0).id);
		}
		else if(key==KEYDOWN.V) {
			Xonomy.pasteElement($element.closest("div.element").get(0).id, "after");
		}
		else if(key==KEYDOWN.UP) {
			Xonomy.moveElementUp($element.closest("div.element").get(0).id);
		}
		else if(key==KEYDOWN.DOWN) {
			Xonomy.moveElementDown($element.closest("div.element").get(0).id);
		}
	}
	else { //control key NOT pressed
		if([KEYDOWN.LEFT, KEYDOWN.UP].indexOf(key)>-1) {
			$("span[tabindex='"+(tabindex-1)+"']").focus();
		}
		else if([KEYDOWN.RIGHT, KEYDOWN.DOWN].indexOf(key)>-1) {
			$("span[tabindex='"+(tabindex+1)+"']").focus();
		}
		else if([KEYDOWN.ENTER].indexOf(key)>-1) {
		 	$element.click();
		}
		else if([KEYDOWN.SPACE].indexOf(key)>-1) {
			$element.click();
		}
	}	
 }
    ...
    if($element.hasClass('menuItem')) {
		if([KEYDOWN.LEFT, KEYDOWN.UP].indexOf(key)>-1) {
			$element.prev('.menuItem').focus();
		}
		else if([KEYDOWN.RIGHT, KEYDOWN.DOWN].indexOf(key)>-1) {
			$element.next('.menuItem').focus();
		}
		else if([KEYDOWN.ENTER, KEYDOWN.SPACE].indexOf(key)>-1) {
		 	$element.click();
		}
	}	
 }
...
Xonomy.render=function(data, editor, docSpec) { //renders the contents of an editor
	...
	if(data.documentElement) data=Xonomy.xml2js(data, null, Xonomy.namespaces);
	...
	$(editor).off("keydown", Xonomy.keyHandler)
	.on("keydown", Xonomy.keyHandler);
	
	Xonomy.refresh();
	Xonomy.validate();
	Xonomy.applyTabIndexes();
};
...
Xonomy.wrap=function(htmlID, param) {
	...
	if(Xonomy.textFromID==Xonomy.textTillID) { //abc --> a<XYZ>b</XYZ>c
		...
		html+=Xonomy.renderElement(Xonomy.xml2js(xml, null, Xonomy.namespaces));
...
Xonomy.plusminus=function(htmlID, forceExpand) {
	...
	if($element.hasClass("collapsed")) {
		...
		if($element.hasClass("oneliner")) $children.fadeIn("fast"); else $children.slideDown(Xonomy.slideSpeed);
	} else if(!forceExpand) {
	...
	if(!Xonomy.notclick) {
		...
		if(what=="rollouter") {
			if( $("#"+htmlID).children(".tag.opening").children(".rollouter").hasClass("rolledout") ) {
				...
				$("#"+htmlID).children(".tag.opening").children(".attributes").addClass("rolledout").hide().slideDown(Xonomy.slideSpeed);
			}
		}
		Xonomy.notclick=true;
	}
...
Xonomy.destroyBubble=function() {
	if(document.getElementById("xonomyBubble")) {
		...
		if(Xonomy.elementIDToFocus) { //return focus to triggering element
			$("div#"+Xonomy.elementIDToFocus+" span.name").first().focus();
		}
	}
};
...
Xonomy.showBubble=function($anchor) {
	...
	$owningElement = $anchor.closest("div.element");
	Xonomy.elementIDToFocus = $owningElement.length>0 ? $owningElement.get(0).id : null; //store id of element to focus after closing the bubble
	...
	$bubble.slideDown(Xonomy.slideSpeed, function() {
		$bubble.find(".focusme").first().focus(); //if the context menu contains anything with the class name 'focusme', focus it.
	});
	$bubble.off("keydown", Xonomy.keyHandler)
	.on("keydown", Xonomy.keyHandler);
};


Xonomy.getBooleanAsker=function(tooltip) {
return function(defaultBool){
	var html="";
	html+="<form onsubmit='Xonomy.answer(this.val.checked?'true':'false'); return false'>";
		html+="<input name='val' class='textbox focusme' value='na' ";
		html+=" type=\"checkbox\" ";
		if(tooltip)
		    html+=" title='"+Xonomy.xmlEscape(tooltip)+"' ";
		if(defaultBool==="true" || defaultBool==="TRUE" || defaultBool===true)
			html+=" checked ";
...
Xonomy.askBoolean=Xonomy.getBooleanAsker();

//type is input type: 'email', 'number', 'date' etc, defaults to 'text'.
//pattern is regex pattern string
Xonomy.getStringAsker=function(tooltip, _type, _pattern) {
	return function(defaultString) {
		var html="";
		var type = _type || "text"; //e.g. email, number, date
		html+="<form>";
			html+="<input name='val' id='val' class='textbox focusme' value='"+Xonomy.xmlEscape(defaultString)+"' ";
			html+=" type=\""+type+"\" ";
			if(tooltip)
				html+=" title=\""+Xonomy.xmlEscape(tooltip)+"\" ";
			if(_pattern)
				html+=" pattern=\""+Xonomy.xmlEscape(_pattern)+"\" ";
			html+="/> <input type='button' value='OK' onclick=\"Xonomy.answer(document.getElementById('val').value)\" />";
		html+="</form>";
		return html;
	}
};
Xonomy.askString=Xonomy.getStringAsker();

Xonomy.getLongStringAsker=function(tooltip, _pattern) {
	return function(defaultString) {
		var html="";
		html+="<form>";
			html+="<textarea name='val' id='val' class='textbox focusme' spellcheck='false' ";
			if(tooltip)
				html+=" title=\""+Xonomy.xmlEscape(tooltip)+"\" ";
			if(_pattern)
				html+=" pattern=\""+Xonomy.xmlEscape(_pattern)+"\" ";
			html+=" >" + Xonomy.xmlEscape(defaultString)+"</textarea>"
			html+="<div class='submitline'><input type='button' value='OK' onclick=\"Xonomy.answer(document.getElementById('val').value)\" ></div>";
		html+="</form>";
		return html;
	}
};
Xonomy.askLongString=Xonomy.getLongStringAsker();

Xonomy.getPicklistAsker = function(tooltip, _picklist) {
	return function(defaultString, picklist) {
		var html="";
		html+="<div class='menu' title='"+(tooltip?tooltip:'')+"' >";
		if(!picklist && _picklist)
			picklist = _picklist;
...
Xonomy.askPicklist=Xonomy.getPicklistAsker();
...
Xonomy.attributeMenu=function(htmlID) {
	...
	for(var i=0; i<spec.menu.length; i++) {
		...
		if(includeIt) {
			html+="<div class='menuItem "+focusmeClass+"' tabindex='1' onclick='Xonomy.callMenuFunction(Xonomy.docSpec.elements[\""+elName+"\"].attributes[\""+name+"\"].menu["+i+"], \""+htmlID+"\")'>";
			html+=Xonomy.formatCaption(Xonomy.textByLang(item.caption));
			html+="</div>";
			focusmeClass = ""; //so only first menu item gets the focusme class
		}
	}
...
Xonomy.elementMenu=function(htmlID) {
	...
	for(var i=0; i<spec.menu.length; i++) {
		...
		if(includeIt) {
			html+="<div class='menuItem "+focusmeClass+"' tabindex='1' onclick='Xonomy.callMenuFunction(Xonomy.docSpec.elements[\""+elName+"\"].menu["+i+"], \""+htmlID+"\")'>";
			html+=Xonomy.formatCaption(Xonomy.textByLang(item.caption));
			html+="</div>";
			focusmeClass = ""; //so only first menu item gets the focusme class
		}
...
Xonomy.newElementChild=function(htmlID, parameter) {
	Xonomy.clickoff();
	var html=Xonomy.renderElement(Xonomy.xml2js(parameter, null, Xonomy.namespaces));
	var $html=$(html).hide();
	$("#"+htmlID+" > .children").append($html);
	Xonomy.plusminus(htmlID, true);
	Xonomy.refresh();
	Xonomy.changed();
	$html.slideDown(Xonomy.slideSpeed);
};
...
Xonomy.editRaw=function(htmlID, parameter) {
	...
	Xonomy.answer=function(val) {		
		...
		else if(parameter.toXml) var jsElement=Xonomy.xml2js(parameter.toXml(val, jsElement), null, Xonomy.namespaces);
		else jsElement=Xonomy.xml2js(val, null, Xonomy.namespaces);		
		var obj=document.getElementById(htmlID);
...
*/
Xonomy.editElementSimpleContent=function(htmlID, parameter){ //edit element content text in a popup
			//build jsElement from html at htmlID
			var div=document.getElementById(htmlID);
			var jsElement=Xonomy.harvestElement(div);
			var txt = jsElement.getText(); //get initial content text
			var askerContent = parameter.asker ? parameter.asker(txt) : Xonomy.askLongString(txt);
			//show bubble (popup) with the asker, using txt as the value to edit
			document.body.appendChild(Xonomy.makeBubble(askerContent)); //create bubble	
			//anchor bubble (popup) to element we're editting
			Xonomy.showBubble($(div));		
			Xonomy.answer=function(val) {				
				//set single text child node
				jsElement.children = [{type: "text", value: Xonomy.xmlEscape(val), htmlID: "", parent: function(){return jsElement}}];					
				//update the ui
				var html=Xonomy.renderElement(jsElement);
				$('#'+htmlID).replaceWith(html);
				Xonomy.clickoff();
...
Xonomy.copyElement=function(htmlID, parameter){
			//harvests the xml text for current element, stores it on Xonomy.elementCopy
			var div=document.getElementById(htmlID);
			var jsElement=Xonomy.harvestElement(div);
			var xmlText=Xonomy.js2xml(jsElement);
			Xonomy.elementCopy = xmlText;
};
Xonomy.pasteElement=function(htmlID, parameter){ //parameter is "before", "child" or "after", default to "child"
			//parameter indicates above/child/below
			var pasteMode = parameter ? parameter : "child";
			if(!Xonomy.elementCopy)
				return;
			var jsElement=Xonomy.xml2js(Xonomy.elementCopy, null, Xonomy.namespaces);		
			var obj=document.getElementById(htmlID);
			var $html=$(Xonomy.renderElement(jsElement));
						
			if(pasteMode=="before")
				$(obj).before($html);
			else if(pasteMode=="child")
				$(obj).find("div.children").first().append($html);
			else if(pasteMode=="after") 
				$(obj).after($html);	
			$html.find("span.name").first().focus(); //focus pasted element
			Xonomy.clickoff();
			Xonomy.refresh();
			Xonomy.changed();
};
Xonomy.moveElementUp=function(htmlID, parameter){
			var $thisEl = $("div#"+htmlID);
			var $sibling = $thisEl.prev();
			if($sibling.length>0) {
			  $sibling.before($thisEl);
			  $thisEl.find("span.name").first().focus(); //focus moved element
			  Xonomy.clickoff();
			  Xonomy.refresh();
			  Xonomy.changed();
			}
};
Xonomy.moveElementDown=function(htmlID, parameter){
			var $thisEl = $("div#"+htmlID);
			var $sibling = $thisEl.next();
			if($sibling.length>0) {
			  $sibling.after($thisEl);
			  $thisEl.find("span.name").first().focus(); //focus moved element
			  Xonomy.clickoff();
			  Xonomy.refresh();
			  Xonomy.changed();
...
//apply correct tabindex values for focus applicable elements
Xonomy.applyTabIndexes=function() {
	var tabindex = 1;
	$("span.opening span.name, div.textnode span.value, span.valueContainer").each(function(){
